    ...
    return new Promise((resolve, reject) => {
        ...
        fetch(reposurl).then(res => res.json()).then((out) => {
            ...
            for (let i = 0; i < out.length; i++) {
                starscount += out[i].stargazers_count
            ...
            document.querySelectorAll('.github-stars-count').forEach(el => el.innerHTML = starscount)
...
function gitpinjson(data, type, element) {
    ...
    if (type == "repo") {
        ...
        if (data.license != null) {
            ...
            span = document.createElement("span")
            span.innerHTML = data.license.name
